      ...
      if (dataOnly) {
        ...
        while (n < x) {
          ...
          if (parseLinks) {
            ...
            op += '<div class="twitter-text"><h5>'+$(authors[n]).find('.p-name').html()+' <span class="tag">'+strip($(authors[n]).find('.p-nickname').html())+'</span></h5><p class="tweet">' + strip(tweets[n].innerHTML) + '</p></div>';
            if (printTime) {
              var timeLabel = times[n].getAttribute('aria-label');
              timeLabel = timeLabel.replace("Posted on", "");
              if (permalinks) {
                op += '<div class="date-time"><span class="timePosted"><a href="' + permalinksURL[n] +
                   '">' + times[n].getAttribute('aria-label') + '</a></span></div>';
              } else {
                op += '<div class="date-time"><span class="timePosted">' +
                    timeLabel + '</span></div>';
              }
            }
          } else {
